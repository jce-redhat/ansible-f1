...
export class GodzillaMode {
    ...
    for (let gx = -CITY_SIZE + GRID_SPACING; gx < CITY_SIZE; gx += GRID_SPACING) {
      for (let gz = -CITY_SIZE + GRID_SPACING; gz < CITY_SIZE; gz += GRID_SPACING) {
        ...
        for (let b = 0; b < bPerBlock; b++) {
          ...
          mesh.position.set(gx + offX, h / 2, gz + offZ);
          this.group.add(mesh);
  ...
  _updateMovement(dt) {
    ...
    this.godzilla.rotation.y = this._facing + Math.PI;
  }
  ...
  _updateWalkAnim(dt) {
    ...
    if (this._parts.jaw) {
      this._parts.jaw.position.y = 7 - Math.abs(Math.sin(this._walkPhase * 0.3)) * 0.15;
    }
  }

  _updateCamera(dt, now) {
    const offset = new THREE.Vector3(
      Math.sin(this._facing + Math.PI) * 25,
      18,
      Math.cos(this._facing + Math.PI) * 25
    );
  ...
  _spawnRubble(building) {
    ...
    const color = building.mesh.material.color.getHex();

    for (let i = 0; i < count; i++) {
      const size = 0.3 + Math.random() * 0.6;
      const geo = new THREE.BoxGeometry(size, size, size);
      const mat = new THREE.MeshStandardMaterial({
        color, roughness: 0.8, transparent: true, opacity: 1,
      });
      const mesh = new THREE.Mesh(geo, mat);
      mesh.position.set(
